 			 Xan's friendship path for BG1
				   Version 11
			  A Pocket Plane Group Project
			   http://www.pocketplane.net

Contents

1. Introduction
2. Installation
3. General Information
4. Credits
5. Version History
6. Legal Information

----------------
1. Introduction
----------------

Before installing the mod, PLEASE read the installation section.

This mod provides a friendship path for Xan NPC in Baldur's Gate I.

Xan, a non-player character from the first game in Baldur's Gate series, is a depressed elven mage - enchanter. Notable quotes: "We're all doomed."
"Life is so hollow." "It is hardly worth the effort of trying."
"What is the point?" "Our quest is vain." "Onward, to futility!"

The mod is compatible with all Baldur's Gate 1 games (BG1 w/o TotSC, BGT, Tutu, BG:EE, EET).

----------------
2. Installation
----------------

Make sure that Baldur's Gate or ANY game-related utilities are not running,
and that you are installing the mod into the correct directory.

Before installing any mods, it is recommended to backup your chitin.key,
baldur.exe, your entire override directory and your dialog.tlk
(in case of BG:EE, lang directory).

Unpack XanBG1Friend_v9.zip into your Baldur's Gate directory.

TO INSTALL, UNINSTALL OR REINSTALL THE MOD, run setup-xanbg1friend.exe.

For Baldur's Gate: Enhanced Edition players: you need to install the mod
in the same folder as your chitin.key file.

-----------------------
3. General Information
-----------------------

Xan's friendship path consists of sixteen sequential dialogues and four
chapter-related talks in chapters 4,5,6 and 7. All talks are Xan-initiated.

The talks trigger if Xan is in party, and the protagonist is either not
eligible for a romance with Xan, or the romance is not active. 

This means that you will still get the friendship path, if your Xan romance
has ended. Exception: if the romance was ended after it has become
committed.

If you do not wish to talk, some banters are skipped. Every banter has a
"I don't want to talk right now"-like option.

Other Xan projects can be found here:

BG1 NPC Project, Xan Romance: http://gibberlings3.net/bg1npc

Xan BG2 NPC: http://www.pocketplane.net/xan

Xan BG2 Voice and Xan BG1 Voice in BG2: http://www.pocketplane.net/audio

-----------
4. Credits 
-----------

Olga Silaeva (Kulyok)				Writer
						Programmer
						Russian translation

Kaeloree, Isaya					BG:EE conversion

Beta testers: Accolon, jastey, grogerson

Russian translation by Kulyok.

German translation by Steffen.

French translation by Eleima, proofreading by Graoumf's workshop.

Italian translation by totuccio5, proofreading by Aedan.

XanBG1Friend was created with the following software:

DLTCEP					http://www.dragonlancetc.com
Infinity Explorer			http://infexp.sourceforge.net
Near Infinity				http://www.idi.ntnu.no/~joh/ni
NSIS					http://nsis.sourceforge.net
WeiDU					http://www.weidu.org

Special thanks to Kaeloree and Isaya for making the mod work on BGEE.

-------------------
5. Version History
-------------------

Version 11
-added .ini with metadata to support ALIEN's Project Infinity
-changed infer_charset -> infer_charsets in tp2
- Updated to Weidu 246

Version 10
-changed to using HANDLE_CHARSET
-EET compatibility added
-updated to WeiDU 240

Version 9
- multi-language BGEE support
- Updated to Weidu 236

Version 8
- Added Italian translation
- Updated to Weidu 23108 for language compatibility with BG:EE

Version 7
- BG:EE compatibility
- A thorough typo hunt
- Updated to Weidu 23107 for language compatibility with BG:EE

Version 6
- Added French translation
- Revised German translation
- Updated to Weidu 211

Version 5
- German translation
- Updated to Weidu 208

Version 4
- Removed MODDER flag
- Updated to Weidu 203

Version 3
- Compatibility with BGT, BG1
- Russian translation

Version 2
- Fixed weights for compatibility with BG1 NPC Project

Version 1
- Initial release

---------------------
6. Legal Information
---------------------

=================================================================================
BALDUR'S GATE II: SHADOWS OF AMN Developed and © 2000 BioWare Corp. All Rights
Reserved. BALDUR'S GATE II: THRONE OF BHAAL Developed and © 2001 BioWare
Corp. All Rights Reserved. Baldur's Gate, Shadows of Amn, Tales of the Sword
Coast, Forgotten Realms, the Forgotten Realms logo, Advanced Dungeons & Dragons,
the AD&D logo, TSR and the TSR logo, and the Wizards of the Coast logo, are
trademarks of the Wizards of the Coast, Inc., a subsidiary of Hasbro, Inc., and
are used by Interplay Entertainment Corp. under license. All Rights Reserved.
BioWare, the BioWare Infinity Engine and the BioWare logo are the trademarks of
BioWare Corp. All Rights Reserved. Black Isle Studios and the Black Isle Studios
logo are trademarks of Interplay Entertainment Corp. All Rights Reserved.
Exclusively licensed and distributed by Interplay Entertainment Corp. All other
trademarks and copyrights are property of their respective owners.
=================================================================================

=================================================================================
XanBG1Friend © 2007 Olga Silaeva. XanBG1Friend is not developed, supported, or
endorsed by BioWare, Black Isle Studios, Interplay Entertainment Corp., or
the  Wizards of the Coast. All other trademarks and copyrights are property of
their respective owners.
=================================================================================

=================================================================================
REDISTRIBUTION NOTE: XanBG1Friend was created to be freely enjoyed by all
Baldur's Gate II players. However, XanBG1Friend may not be sold, published,
compiled or redistributed in any form without the consent of its author.
=================================================================================